import datetime
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QTableWidget, QTableWidgetItem, QHeaderView, QTabWidget, QAbstractItemView
...
class TestDataManager:
    def create_test_table(self, categories_to_include):
        """Builds a tab widget containing tables for only the specified categories."""
        tabs = QTabWidget()
        tabs.setStyleSheet("""
            QTabBar::tab { padding: 8px 15px; font-weight: bold; background: #F8F9FA; border: 1px solid #E4E7ED; margin-right: 2px;}
            QTabBar::tab:selected { background: white; color: #409EFF; border-bottom: none; }
            QTabWidget::pane { border: none; }
        """)
        
        for cat_name in categories_to_include:
            if cat_name not in TEST_CATEGORIES: continue
            tests = TEST_CATEGORIES[cat_name]
            
            tab_widget = QWidget()
            t_layout = QVBoxLayout(tab_widget)
            t_layout.setContentsMargins(0, 10, 0, 0)
            
            # Run All Button Layout
            top_layout = QHBoxLayout()
            top_layout.addStretch()
            run_all_btn = QPushButton("▶ 현재 탭 전체 테스트 자동 실행")
            run_all_btn.setStyleSheet("background-color: #67C23A; color: white; padding: 6px 15px; border-radius: 4px; font-weight: bold;")
            top_layout.addWidget(run_all_btn)
            t_layout.addLayout(top_layout)

            table = QTableWidget(len(tests), 6)
            table.setHorizontalHeaderLabels(["테스트 항목", "대상", "테스트 내용", "기대 결과", "상태 / 실행", "최근 테스트 기록"])
            ...
            for row, item_tup in enumerate(tests):
                ...
                # Default Empty History Item
                hist_item = QTableWidgetItem("-")
                hist_item.setTextAlignment(Qt.AlignCenter)
                table.setItem(row, 5, hist_item)
                
                def make_run_func(b, s, h_item):
                    def run_click():
                        b.setText("진행 중")
                        b.setEnabled(False)
                        ...
                        def finish():
                            b.setText("완료✅")
                            b.setEnabled(True) # re-enable for next runs
                            ...
                            h_item.setText(f"✅ PASS ({now_str})")
                        QTimer.singleShot(800, finish)
                    return run_click
                
                click_fn = make_run_func(btn, status_lbl, hist_item)
                btn.clicked.connect(click_fn)
                run_buttons.append(click_fn)
            ...
            def make_run_all(btns):
                def trigger_all():
                    for i, b_fn in enumerate(btns):
                        # slightly stagger execution for effect
                        QTimer.singleShot(i * 300, b_fn)
                return trigger_all
            
            run_all_btn.clicked.connect(make_run_all(run_buttons))
            
            tabs.addTab(tab_widget, cat_name)
